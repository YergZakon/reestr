...
/** GET /api/admin/biz-stats?days=30 — посещаемость бизнес-навигатора (только МНЭ).
 *  Источник — biz_event (без персданных: IP не хранится, cookie не ставятся). */
export async function GET(req: NextRequest) {
  const user = await getCurrentUserWithAccess();
  if (!user) return NextResponse.json({ error: "Не авторизован" }, { status: 401 });
  if (!isMne(user.role)) return NextResponse.json({ error: "Доступ только для МНЭ" }, { status: 403 });
