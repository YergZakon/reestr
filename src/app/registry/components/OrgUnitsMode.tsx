...
export default function OrgUnitsMode({ me, lang = "ru" }: { me: Me | null; lang?: Lang }) {
  const t = MDICT[lang];
  ...
  const create = async () => {
    ...
    if (!parentId || !name.trim() || !code.trim()) { setErr(t.umFillAll); return; }
    ...
    try {
      ...
      if (!res.ok) { setErr(d.error || t.umCreateFail); return; }
      setMsg(t.umCreated(name.trim()));
      setName(""); setShortName(""); setCode(""); setCodeTouched(false);
      load();
    } catch { setErr(t.umReqFail); } finally { setBusy(false); }
  };
  ...
  const patch = async (body: Record<string, unknown>, okMsg: string) => {
    ...
    try {
      ...
      const d = await res.json();
      if (!res.ok) { setErr(d.error || t.umPatchFail); return; }
  ...
  return (
    <div className="reg-mon">
      <h1 className="reg-cat-h1">{t.umH1}</h1>
      ...
      <div className="reg-cost-params" style={{ marginTop: 18 }}>
        <div className="reg-cost-params-h"><span>{t.umNew}</span></div>
        <div className="reg-cost-params-grid">
          <div className="reg-cost-param">
            <span className="reg-cost-param-l">{t.umParent}</span>
            <select value={parentId} onChange={(e) => { setParentId(e.target.value ? Number(e.target.value) : ""); setCodeTouched(false); }}
              ...
              <option value="">{t.umPick}</option>
              {parentOptions.map((o) => (
                <option key={o.id} value={o.id}>{o.short_name || o.name_ru} · {t.umTypeLabel[o.type] || o.type}</option>
              ))}
            </select>
          </div>
          <div className="reg-cost-param">
            <span className="reg-cost-param-l">{t.umName}</span>
            <div className="reg-cost-param-in"><input value={name} onChange={(e) => setName(e.target.value)} placeholder={t.umNamePh} /></div>
          </div>
          <div className="reg-cost-param">
            <span className="reg-cost-param-l">{t.umShort}</span>
            <div className="reg-cost-param-in"><input value={shortName} onChange={(e) => setShortName(e.target.value)} placeholder={t.umShortPh} /></div>
          </div>
          <div className="reg-cost-param">
            <span className="reg-cost-param-l">{t.umCode}</span>
      ...
      {/* структура */}
      <div className="reg-biz-blockh" style={{ marginTop: 26 }}>{t.umStructure(mine.length)}</div>
      <div className="reg-mon-tablewrap">
        <table className="reg-mon-table">
          <thead>
            <tr><th>{t.umThName}</th><th>{t.umThType}</th><th>{t.umThCode}</th><th>{t.umThUsers}</th><th>{t.umThNpa}</th><th>{t.umThReqs}</th><th>{t.umThActions}</th></tr>
          </thead>
          <tbody>
            {tree.map(({ node, depth }) => {
              ...
              return (
                <tr key={node.id}>
                  <td className="t-name" style={{ paddingLeft: 12 + depth * 20 }}>
                    ...
                    {editId === node.id ? (
                      <span style={{ display: "inline-flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                        ...
                        <input value={editShort} onChange={(e) => setEditShort(e.target.value)} placeholder={t.umBriefPh}
                        ...
                        <button className="reg-rev-confirm" style={{ height: 30 }} disabled={busy}
                          onClick={() => patch({ id: node.id, name_ru: editName.trim(), short_name: editShort.trim() || null }, t.umRenamed)}>{t.umOk}</button>
                  ...
                  <td>{t.umTypeLabel[node.type] || node.type}</td>
                  ...
                  <td className="nowrap">
                    {editable && editId !== node.id && (
                      <>
                        <button className="reg-permit-more" onClick={() => { setEditId(node.id); setEditName(node.name_ru); setEditShort(node.short_name || ""); }}>{t.umRename}</button>
                        {node.parent_id != null && (
                          <button className="reg-permit-more" style={{ color: "#C0392B", marginLeft: 10 }} disabled={busy}
                            onClick={() => patch({ id: node.id, active: false }, t.umDisabled)}>{t.umDisable}</button>
                        )}
                      </>
                    )}
                  </td>
        ...
        {!tree.length && <div className="reg-empty">{t.umNoAccess}</div>}
      </div>

      <div className="reg-help-note" style={{ marginTop: 8 }}>
        <b>{t.umHelpB}</b>
        {t.umHelp1}<b>{t.umHelpAnalysts}</b>{t.umHelp2}<b>{t.umHelpAssign}</b>{t.umHelp3}
      </div>
    </div>
  );
}
