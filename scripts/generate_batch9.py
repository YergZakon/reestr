"""
generate_batch9.py — генератор карточек для батча 4.

Берёт _candidates_batch9.json и для каждого фрагмента строит карточку
...
Запуск:
    python generate_batch9.py
Создаёт 4 файла в outputs/cards/batch_009{a,b,c,d}_*.json.
"""
...
INPUT = PROJECT_ROOT / "outputs" / "cards" / "_candidates_batch9.json"
OUT_DIR = PROJECT_ROOT / "outputs" / "cards"

START_CODE = 1486   # следующий номер после пилота 112
...
def main() -> None:
    ...
    for k, cards in by_split.items():
        out_path = OUT_DIR / f"batch_009{suffixes[k]}_{k}.json"
