"""
generate_batch4.py — генератор карточек для батча 4.

Берёт _candidates_batch4.json и для каждого фрагмента строит карточку
...
Запуск:
    python generate_batch4.py
Создаёт 4 файла в outputs/cards/batch_004{a,b,c,d}_*.json.
"""
...
INPUT = PROJECT_ROOT / "outputs" / "cards" / "_candidates_batch4.json"
OUT_DIR = PROJECT_ROOT / "outputs" / "cards"

START_CODE = 113   # следующий номер после пилота 112
...
# Расширенный список — обновляется по мере обнаружения новых паттернов в реестре
ORGAN_PATTERNS = [
    # Органы и их структуры
    r"\bместн\w+ исполнительн\w+ орган",
    r"\bкомитет\w+ управления",
    r"\bструктурным подразделением",
    r"\bорганом, осуществляющим функции",

    # Принятие решений и формирование документов
    r"\b(Принятие|Вынесение)\s+решения",
    r"\bФормирование\s+(перечня|реестра|списка)",
    r"\bПодготовка\s+(комплекта|договора|заключения|акта)",
    r"\bУтверждение\s+(перечня|правил|комплекта|регламента|списка)",

    # Соблюдение сроков ОРГАНОМ при оказании госуслуг
    r"\bСоблюдение\s+срок\w*\s+(вручен|выдач|направлен|оформлен|подготовк|регистраци|внесен|оказан|предоставлен|совершен|формирован|размещен|рассмотрен|приняти|заключен|передач|проведен)",
    r"\bСоблюдение\s+срок\w*\s+(аукциона|конкурса|торгов)",
    r"\bСоблюдение\s+установленных\s+сроков\s+рассмотрения",
    r"\bСоблюдение\s+(согласующими\s+органами|комиссией)",
    r"\bСоблюдение.*\bоргана?м\b",
    r"\bСоблюдение.*срок.*местным исполнительным",

    # Конкретные действия органа в госуслугах
    r"\bвручен\w+\s+копи\w+\s+решения\b.*\bзаявител",
    r"\bВручение\b.*\bзаявителю",
    r"\bПередача заявителю\b",
    r"\bпередач\w+\s+заявителю",
    r"\bОформление\s+(акта\s+о\s+выборе|договора)",
    r"\bНаправление\s+(копии решения|соответствующих запросов)",
    r"\bРазмещение информации\b",
    r"\bПредоставление\s+информации\s+о\s+наличии\s+земельных\s+участков",
    r"\bРассмотрение\s+(заявления|предложения|ходатайств)",

    # Ведение реестров, кадастров, систем учёта
    r"\bВедение\s+(автоматизированной|информационной|государственной|реестра|кадастра|книги|журнала|банка)",
    r"\bВнесение\s+(в книгу|в реестр|записей|изменений|сведений в)",
    r"\bУчёт\s+(количества|качества)\s+земель",
    r"\bУчет\s+(количества|качества)\s+земель",
    r"\bНакопление,\s+обработка\s+и\s+ведение",

    # Аукционы, торги, конкурсы — процедура органа
    r"\bпроведения?\s+(торгов|аукционов|конкурсов)",
    r"\bаукциона\s+по\s+продаже\s+земельных\s+участков",
    r"\bРазмещение\s+объявления\s+о\s+проведении\s+торгов",

    # Госуслуги, документы органа
    r"\bдоговора\s+временного\s+землепользования",
    r"\bзаключения\s+земельной\s+комиссии\b.*\bподготовк",
    r"\bПодготовка комплекта",
    r"\bупорядочение",
    r"\bАИС\s+государственного",
]
...
def main() -> None:
    ...
    for k, cards in by_split.items():
        out_path = OUT_DIR / f"batch_004{suffixes[k]}_{k}.json"
