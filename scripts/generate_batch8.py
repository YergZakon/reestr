"""
generate_batch8.py — генератор карточек для батча 4.

Берёт _candidates_batch8.json и для каждого фрагмента строит карточку
...
Запуск:
    python generate_batch8.py
Создаёт 4 файла в outputs/cards/batch_008{a,b,c,d}_*.json.
"""
...
INPUT = PROJECT_ROOT / "outputs" / "cards" / "_candidates_batch8.json"
OUT_DIR = PROJECT_ROOT / "outputs" / "cards"

START_CODE = 1105   # следующий номер после пилота 112
...
def main() -> None:
    ...
    for k, cards in by_split.items():
        out_path = OUT_DIR / f"batch_008{suffixes[k]}_{k}.json"
