"""
generate_batch6.py — генератор карточек для батча 4.

Берёт _candidates_batch6.json и для каждого фрагмента строит карточку
...
Запуск:
    python generate_batch6.py
Создаёт 4 файла в outputs/cards/batch_006{a,b,c,d}_*.json.
"""
...
INPUT = PROJECT_ROOT / "outputs" / "cards" / "_candidates_batch6.json"
OUT_DIR = PROJECT_ROOT / "outputs" / "cards"

START_CODE = 508   # следующий номер после пилота 112
...
def main() -> None:
    ...
    for k, cards in by_split.items():
        out_path = OUT_DIR / f"batch_006{suffixes[k]}_{k}.json"
